...
public class FlightListActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        makeNonExistingPref("list_pref_indicator", true);
        makeNonExistingPref("list_pic_size", "3840");
        makeNonExistingPref("list_pic_cache", "Full");
        ...
        mNavItems.add(new NavItem(R.id.action_prefpic, R.string.main_menu_prefpic, R.drawable.ic_photo_grey600_36dp));
    ...
    private void makeNonExistingPref(String key, boolean value) {
        Editor edit = sharedPrefs.edit();
        if (!sharedPrefs.contains(key))
    ...
    private boolean onMenuItemSelected(int id) {
        ...
        switch (id) {
            ...
            case R.id.action_prefdetail:
                ...
                break;
            case R.id.action_prefpic:
                if (mTwoPane) {
                    getSupportFragmentManager().beginTransaction()
                            .replace(R.id.flight_detail_container, new PicPreferenceFragment()).commit();
                    setTitle(getString(R.string.title_pic_pref));
                } else {
                    Intent listDetailActivity = new Intent(getApplicationContext(),
                            FragActivity.class).putExtra("FragType", PIC_PREF);
                    startActivity(listDetailActivity);
                }
                break;
