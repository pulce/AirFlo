...
public class FlightData {
	...
	public static List<FlightDataItem> ITEMS = new ArrayList<>();
	public static Map<String, FlightDataItem> ITEM_MAP = new HashMap<>();
	...
	public static class FlightDataItem {
		...
		public Map<String, String> sortContent = new HashMap<>();
		...
		public String specialOp(String key, String data) {
			...
			if (key.equals("starttime") && data.length() > 5) {
				modData = addTime(data, getFromKey("UTCorr"));
				putContent("UTCorr", data);
			}
			if (key.equals("duration")) {
				putContent("landingtime",
		...
		private static String addTime(String base, String toAdd) {
			...
			try {
				...
				long moreMillis = ((dur.getHours()*60+dur.getMinutes())*60+dur.getSeconds())*1000;
		...
		private String transform(String compKey, String itemData) {
			...
			if (compType.equals("date")) {
				try {
					Date dt = dateParser.parse(itemData);
