...
public class CheckForUpdates extends AsyncTask<String, Void, Boolean> {
	...
	protected Boolean doInBackground(String... org) {
		String tag;
		try {
			URLConnection con = new URL(FlightListActivity.APPURL).openConnection();
			con.connect();
		...
		if (tag.equals(FlightListActivity.VERSIONID)) {
			answer = context.getString(
					R.string.already_latest_version)
					+ " " + FlightListActivity.VERSIONID + ".";
			return false;
		}
		answer = "Version "
				+ tag
	...
	protected void onPostExecute(Boolean newVersion) {
		...
		if (newVersion) {
			Intent browse = new Intent(Intent.ACTION_VIEW,
					Uri.parse(FlightListActivity.APPURL));
